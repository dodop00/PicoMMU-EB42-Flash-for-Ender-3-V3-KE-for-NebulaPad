...
from . import prtouch_probe as prtouch_prtouch_probe
import math
import random
import threading
...
PR_ERR_CODE_PRES_LOST_RUN_DATA      = {'code':'key401', 'msg':'PR_ERR_CODE_PRES_LOST_RUN_DATA: The pressure data is lost when the prtouch_probe is over and waiting for the data to be sent back.', 'values':[]}
...
PR_ERR_CODE_STEP_LOST_RUN_DATA      = {'code':'key401', 'msg':'PR_ERR_CODE_STEP_LOST_RUN_DATA: The motor step data is lost when the prtouch_probe is over and waiting for data return', 'values':[]}
...
PR_ERR_CODE_G28_Z_DETECTION_TIMEOUT = {'code':'key401', 'msg':'PR_ERR_CODE_G28_Z_DETECTION_TIMEOUT: G28 Z prtouch_probe timed out.', 'values':[]}
PR_ERR_CODE_G29_Z_DETECTION_TIMEOUT = {'code':'key401', 'msg':'PR_ERR_CODE_G29_Z_DETECTION_TIMEOUT: G29 Z prtouch_probe timed out.', 'values':[]}
...
class PRTouchEndstopWrapper:
    def __init__(self, config):
        ...
        self.jump_prtouch_probe_ready = False
        ...
        self.prtouch_probe_min_3err     = config.getfloat('prtouch_probe_min_3err', 0.1, 0.01, 1)
    ...
    def _build_step_config(self):
        ...
        self.prtouch_probe      = self.printer.lookup_object("prtouch_probe")
    ...
    def multi_prtouch_probe_begin(self):
        pass

    def multi_prtouch_probe_end(self):
        pass

    def query_endstop(self, print_time):
        return False
    
    def prtouch_probe_prepare(self, hmove):
        pass

    def prtouch_probe_finish(self, hmove):
        pass
    #endregion
    ...
    def prtouch_probe_ready(self):
        if self.jump_prtouch_probe_ready:
            self.jump_prtouch_probe_ready = False
            return False
        self.ck_g28ed()
        ...
        for i in range(4):
            ...
            self.rdy_pos[i][2] = self.run_step_prtouch(self.g29_down_min_z, self.prtouch_probe_min_3err, True, 5, 3, True)
            pass
        self.print_msg('RDY_POS', "[00=%.2f, 01=%.2f, 11=%.2f, 10=%.2f]" % (self.rdy_pos[0][2], self.rdy_pos[1][2], self.rdy_pos[2][2], self.rdy_pos[3][2]))
        return True

    def run_step_prtouch(self, down_min_z, prtouch_probe_min_3err, rt_last=False, pro_cnt=3, crt_cnt=3, fast_prtouch_probe=False, tri_min_hold=None, tri_max_hold=None):
        ...
        for i in range(pro_cnt):
            ...
            can_rt = True if can_rt else (i == (crt_cnt - 1)) and (max(res_z) - min(res_z) <= prtouch_probe_min_3err)
            ...
            if fast_prtouch_probe and pro_cnt > 2: 
    ...
    def bed_mesh_post_proc(self, last_point):
        ...
        res1 = self.bed_mesh.bmc.prtouch_probe_helper.results
    ...
    def check_bed_mesh(self, auto_g29=True):
        ...
        self.prtouch_probe_ready()
        self.jump_prtouch_probe_ready = True
        if self.use_adc:
        ...
        if mesh:
            ...
            for i in range(4):
                ...
                self.print_msg('CK_BED_MESH', 'P%d = [x=%.2f, y=%.2f, mest_z=%.2f, prtouch_probe_z=%.2f, err_z=%.2f]' % (i, self.rdy_pos[i][0], self.rdy_pos[i][1], mesh_z, self.rdy_pos[i][2], errs[i]))    
            if err_cnt < 2:
    ...
    def run_G28_Z(self): 
        ...
        # 3. Normal prtouch_probe z
        self.move(now_pos1, self.rdy_xy_spd / 5)
        self.shake_motor(self.shake_cnt)
        res_z = self.run_step_prtouch(self.g29_down_min_z, self.prtouch_probe_min_3err, False, 5, 5, True)
    ...
    def run_G29_Z(self):
        ...
        if self.g29_cnt == 0:
            if self.use_adc:
                self.set_fan_speed('heater_fan', self.fan_heat_min_spd)
            self.set_fan_speed('fan', 0.0)
            if self.prtouch_probe_ready():
                self.run_to_next(now_pos, True)
                self.shake_motor(self.shake_cnt)
        
        self.g29_cnt += 1
        
        now_pos[2] = self.run_step_prtouch(self.g29_down_min_z, self.prtouch_probe_min_3err, True, 5, 3, True)
        self.set_step_par(load_sys=True)
        ...
        if self.g29_cnt == x_cnt * y_cnt:
            ...
            res_z = self.run_step_prtouch(self.g29_down_min_z, self.prtouch_probe_min_3err, True, 5, 3, True)
...
def load_config(config):
    ...
    config.get_printer().add_object('prtouch_probe', prtouch_probe.Printerprtouch_probe(config, vrt))
    return vrt
